            ...
            >
              <Dialog.Panel className="relative px-4 pt-5 pb-4 overflow-hidden text-left transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                ...
                <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                  <button
                    type="button"
                    className={`inline-flex justify-center w-full px-4 py-2 text-base font-medium text-white bg-${confirmationButtonColor}-600 border border-transparent rounded-md shadow-sm hover:bg-${confirmationButtonColor}-700 focus:outline-none focus:ring-2 focus:ring-${confirmationButtonColor}-500 focus:ring-offset-2 sm:ml-3 sm:w-auto sm:text-sm`}
                    onClick={() => {
                      setOpen({
                        id: null,
                        status: false,
                      });
                      onDelete ? onDelete() : onSend() ;
                    }}
                  >
